...
### ❌ Why doesn’t this update the original struct?
...
✅ Tip: When in doubt, ask yourself — *am I modifying the value itself, or the data it points to?*



## How copy slices ?

this is **super important in Go** 👍
Short answer: **you must copy the underlying array**, not just assign the slice.

---

## ❌ Wrong way (this does NOT copy)
...
## ✅ Deep copy vs shallow copy (important idea)
...
## ⚠️ Special case: slice of structs or pointers
...
You’ll need a **manual deep copy**.
...
A **struct tag** in Go is metadata attached to a struct field, written as a raw string literal right after the field declaration. It lets packages like `encoding/json` or `database/sql` map fields to keys, columns, etc.
...
````
Here `json:"id"` tells the JSON encoder to use `id` instead of `ID` when serializing.

Use case: control how fields are marshaled/unmarshaled, validate inputs, or bind request parameters to struct fields.

Struct tags are read at runtime via reflection. The `reflect.StructTag` value attached to each field is accessible with `reflect.Type.Field(i).Tag`, so libraries like `encoding/json` or Gin’s binding can inspect those tags every time a struct instance is serialized, validated, or bound.
...
func main() {
    ...
    field := t.Field(0)
    fmt.Println(field.Tag.Get("json")) // prints "id"
}
````
This pattern lets code interpret the metadata and map fields to JSON keys, form parameters, database columns, etc., during each request or operation.
